...
#include <sign_sensor_ta.h>

static void usage(int argc, char *argv[])
{
	const char *pname = "sign_sensor";

	if (argc)
		pname = argv[0];

	fprintf(stderr, "usage: %s <key_size> <data> <lat> <lon> <time> \n", pname);
	exit(1);
}
		     ...
		     size_t *inbuf_len)
{
	...
	if (argc != 6) {
		warnx("Unexpected number of arguments %d (expected 5)",
		      argc - 1);
		usage(argc, argv);
	}
	...
	sense = strtol(argv[2], &ep, 0);
	lat = strtol(argv[3], &ep, 0);
	lon = strtol(argv[4], &ep, 0);
	time = strtol(argv[5], &ep, 0);
	...
	*key_size = ks;
	...
	char data[100];
	sprintf(data, "%d,%d,%d,%d", sense, lat, lon, time); 
	*inbuf = data;
	*inbuf_len = strlen(data);
...
int main(int argc, char *argv[])
{
	...
	size_t inbuf_len;
	size_t n;
	const TEEC_UUID uuid = TA_SIGN_SENSOR_UUID;
	...
	res = TEEC_InvokeCommand(&sess, TA_SIGN_SENSOR_CMD_GEN_KEY, &op, &eo);
	if (res)
		teec_err(res, eo, "TEEC_InvokeCommand(TA_SIGN_SENSOR_CMD_GEN_KEY)");
	...
	res = TEEC_InvokeCommand(&sess, TA_SIGN_SENSOR_CMD_ENCRYPT, &op, &eo);
	if (eo != TEEC_ORIGIN_TRUSTED_APP || res != TEEC_ERROR_SHORT_BUFFER)
		teec_err(res, eo, "TEEC_InvokeCommand(TA_SIGN_SENSOR_CMD_ENCRYPT)");
