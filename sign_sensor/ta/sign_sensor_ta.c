...
static TEE_Result cmd_gen_key(struct sign_sensor *state, uint32_t pt,
			      TEE_Param params[TEE_NUM_PARAMS])
{
	...
	DMSG("TEE_AllocateTransientObject(%" PRIx32 ", %" PRId32 ")", key_type, key_size);
	...
	DMSG("TEE_GenerateKey(%" PRIx32 ", %" PRId32 ")",key, key_size);
...
static TEE_Result cmd_enc(struct sign_sensor *state, uint32_t pt,
			  TEE_Param params[TEE_NUM_PARAMS])
{
	TEE_Result res;
	const void *inbuf;
...
{
	struct sign_sensor *state = session;
...
{
	switch (cmd) {
	case TA_SIGN_SENSOR_CMD_GEN_KEY:
		return cmd_gen_key(session, param_types, params);
	case TA_SIGN_SENSOR_CMD_ENCRYPT:
