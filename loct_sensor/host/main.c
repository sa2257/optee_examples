...
#include <loct_sensor_ta.h>

static void usage(int argc, char *argv[])
{
	const char *pname = "loct_sensor";

	if (argc)
		pname = argv[0];

	fprintf(stderr, "usage: %s <key_size> <data>\n", pname);
	exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	size_t inbuf_len;
	size_t n;
	const TEEC_UUID uuid = TA_LOCT_SENSOR_UUID;
	...
	res = TEEC_InvokeCommand(&sess, TA_LOCT_SENSOR_CMD_GEN_KEY, &op, &eo);
	if (res)
		teec_err(res, eo, "TEEC_InvokeCommand(TA_LOCT_SENSOR_CMD_GEN_KEY)");
	...
	res = TEEC_InvokeCommand(&sess, TA_LOCT_SENSOR_CMD_ENCRYPT, &op, &eo);
	if (eo != TEEC_ORIGIN_TRUSTED_APP || res != TEEC_ERROR_SHORT_BUFFER)
		teec_err(res, eo, "TEEC_InvokeCommand(TA_LOCT_SENSOR_CMD_ENCRYPT)");
