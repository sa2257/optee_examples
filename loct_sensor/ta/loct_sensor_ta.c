...
static TEE_Result cmd_gen_key(struct loct_sensor *state, uint32_t pt,
...
static TEE_Result cmd_enc(struct loct_sensor *state, uint32_t pt,
			  TEE_Param params[TEE_NUM_PARAMS])
{
	TEE_Result res;
	const void *inbuf;
...
{
	struct loct_sensor *state = session;
...
{
	switch (cmd) {
	case TA_LOCT_SENSOR_CMD_GEN_KEY:
		return cmd_gen_key(session, param_types, params);
	case TA_LOCT_SENSOR_CMD_ENCRYPT:
